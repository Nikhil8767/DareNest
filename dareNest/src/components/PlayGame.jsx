...
export default function PlayGame() {
  ...
  // 🎲 Start countdown instead of direct fetch
  const startGame = () => {
    if (isCounting || loading) return;
    setDare("");
    setCountdown(5);
    setIsCounting(true);
  };
  ...
  return (
    ...
    >
      ...
      {/* 🎴 Card */}
      <div className="relative w-full max-w-lg p-10 rounded-3xl shadow-2xl text-center bg-white/25 backdrop-blur-xl text-white">
        <h1 className="text-4xl font-extrabold mb-4">
          {sessionType === "friends" ? "🎉 Dare Time!" : "❤️ Your Dare"}
        </h1>

        {/* Dare / Countdown Box */}
        <div className="min-h-[120px] flex items-center justify-center text-3xl font-bold p-6 rounded-2xl mb-8 bg-black/30">
          {isCounting && countdown !== null
            ? countdown
            : loading
            ? "🎲 Picking a dare..."
            : dare || "Click below to begin"}
        </div>
        ...
        <button
          ...
          className={`w-full py-3 rounded-xl font-bold text-lg transition-transform hover:scale-105 disabled:opacity-50 ${
            sessionType === "friends"
              ? "bg-yellow-400 text-black hover:bg-yellow-300"
              : "bg-pink-500 hover:bg-pink-600"
          }`}
